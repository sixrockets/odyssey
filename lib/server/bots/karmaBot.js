...
module.exports = function (app) {
  ...
  var MessageParser = require('./karmaBot/parser')(app.modules.BaseParser);

  var KarmaBot = function KarmaBot(app) {
  ...
  KarmaBot.prototype.showKarma = function (channelId, cb) {
    ...
    var query = app.models.User.find().sort([['karma', 'descending']]).limit(5);
  ...
  KarmaBot.prototype.canPerformAction = function (userAction) {
    ...
    if (_.contains(this.globalCommands, userAction.action)) {
      actionKey += userAction.action;
    } else {
      actionKey += userAction.action + ":" + userAction.userName;
    }
  ...
  KarmaBot.prototype._tryAction = function (messageInfo, cb) {
    ...
    var parsedInfo = this.messageParser.parseMessage(messageInfo),
